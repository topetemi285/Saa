...
//       <div
//         className={clsx(
//           "g5 -bottom-7 -top-7 left-0 right-0 -z-1 rounded-3xl opacity-0 transition-opacity-500 absolute",
//           active && "opacity-100"
//         )}
//       >
...
const FaqItem = ({ item, index }) => {
  ...
  return (
    <div className="relative z-2 mb-16">
      ...
      {/* Framer Motion slide down effect */}
      <motion.div
        initial={{ height: 0, opacity: 0 }} // Initial hidden state
        animate={{
          height: activeId === item.id ? "auto" : 0, // Expands or collapses
          opacity: activeId === item.id ? 1 : 0, // Fades in or out
        }}
        transition={{ duration: 0.5, ease: "easeInOut" }} // Smooth transition
        style={{ overflow: "hidden" }} // Prevents overflow when collapsed
      >
        {activeId === item.id && (
          <div className="body-3 px-7 py-3.5">{item.answer}</div>
        )}
